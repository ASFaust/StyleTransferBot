...
class FacebookComment:
    def __init__(self, data, log):
        ...
        try:
            ...
            self.user_name = str(unicodedata.normalize('NFKD', self.data['from']['name']).encode('ascii', 'ignore'))[
                             2:-1]
...
def comment_image(post_id,text,filepath,token):
        params = (
            ('message', text),
            ('access_token', token),
        )
        files = {'source': (filepath.split("/")[-1], open(filepath, "rb"), 'image/png', {'Expires': '0'})}
        response = requests.post('https://graph.facebook.com/v3.2/' + post_id + '/comments', params=params,files=files)
...
class FacebookPost:
    ...
    def comment_image(self, path, text=""):
        ...
        files = {'source': (path.split("/")[-1], open(path, "rb"), 'image/png', {'Expires': '0'})}
        try:
            response = requests.post('https://graph.facebook.com/v3.2/' + self.id + '/comments', params=params,
                                     files=files)
